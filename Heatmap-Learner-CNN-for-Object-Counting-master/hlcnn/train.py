...
class nllloss(nn.Module):
    ...
    # def forward(self, y_pred, y, num_car):        # 这最后一个参数没用到啊
...
optimizer_ft = optim.Adam(model.parameters(), lr=0.00015)   # 预设0.0001
scheduler = torch.optim.lr_scheduler.StepLR(optimizer_ft, step_size=20, gamma=0.1)
model.train()

for epoch in range(35):     # 预设35

    scheduler.step(epoch)
    for batch_idx, (idx, data, GAM, numCar) in enumerate(train_loader):
