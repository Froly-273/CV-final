...
class CARPK(data.Dataset):
    ...
    def __init__(self, root, set, train = True):
        ...
        # self.ids：CARPK/ImageSets/test.txt(train.txt)的每一行
    ...
    def __getitem__(self, index):
        idx = self.ids[index]
        # idx = '164'
        path = os.path.join(self.path_images, idx)

        img = Image.open(os.path.join(path + '.png')).convert('RGB')
        if self.train:
            # 有50%概率做color jitter数据增强
        ...
        annoFile = open('%s/Annotations/%s.txt' % (self.path_devkit, idx), 'r')
