...
def evaluate(downsampling_ratio=8):
    ...
    RAE = 0  # RAE的评价指标，极差
    with torch.no_grad():
        for batch_idx, (idx, im, GAM, numCar) in enumerate(test_loader):
            id_= batch_idx
            image = im.cuda()
            MAP = model(image)
            ...
            gRi = gRi + fark*fark
            ind = ind + 1
            RAE = max(RAE, abs(fark))

            print(idx[0],'\t', np.sum(peakMAP), int(numCar[0]), '\tAE: ', abs(fark))
            ...
            if batch_idx >= 0:
                ...
                ima.save("res1/heatmap-" + idx[0] + ".bmp")
                peakI.save("res1/peakmap-" + idx[0] + ".bmp")
